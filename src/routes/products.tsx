...
import { applications, products, productTypes, brands } from "@/data/products";
...
export const Route = createFileRoute("/products")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: `Product Catalogue — ${site.name}` },
      {
        property: "og:description",
        content: "Search and filter our complete range of interior materials, fittings and tools.",
      },
...
function ProductsPage() {
  ...
  const subcategoryOptions = useMemo(
    () =>
      [
        ...new Set(
          products.filter((p) => !category || p.category === category).map((p) => p.subcategory),
        ),
      ].sort(),
    [category],
  );
  ...
  const results = useMemo(() => {
    ...
    const filtered = products.filter((p) => {
      ...
      if (!q) return true;
      return [p.name, p.subcategory, p.description, p.type, p.application]
        .join(" ")
        .toLowerCase()
        .includes(q);
    });
    const rank = (tag: "featured" | "new" | "popular") => (p: (typeof products)[number]) =>
      p.tags?.includes(tag) ? 0 : 1;
    const key = sort === "New Arrivals" ? "new" : sort === "Popular" ? "popular" : "featured";
    return [...filtered].sort((a, b) => rank(key)(a) - rank(key)(b));
  }, [query, category, subcategory, brand, type, application, sort]);
    ...
    <>
      <section className="bg-walnut-deep py-16 text-primary-foreground lg:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <h1 className="mt-3 font-display text-4xl sm:text-5xl">Explore Our Products</h1>
          <p className="mt-4 max-w-2xl text-primary-foreground/75">
            Materials, fittings, accessories and tools for every project. Prices are shared on request —
            send an enquiry and our team will respond with pricing and availability.
          </p>
          <div className="relative mt-8 max-w-xl">
            <Search className="absolute top-1/2 left-4 h-5 w-5 -translate-y-1/2 text-muted-foreground" aria-hidden />
            <Input
              value={query}
          ...
          <div>
            {results.length ? (
              ...
              <div className="rounded-md border border-dashed border-border p-16 text-center">
                <h2 className="text-lg">No products match your search</h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  Try a different keyword or clear the filters — we stock much more than is listed online.
                </p>
  ...
  return (
    <div>
      ...
      {options.length === 0 && emptyNote ? (
        <p className="mt-1.5 text-xs text-muted-foreground">{emptyNote}</p>
      ) : null}
    </div>
  );
}
